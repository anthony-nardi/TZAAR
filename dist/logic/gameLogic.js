...
import { drawCoordinates, drawGameBoardState, renderInitializingBoard, } from "../rendering/renderHelpers";
...
let botOne = new BotFactory();
let botTwo = new BotFactory();
export function moveAI() {
    const currentTurn = GameState.getCurrentTurn();
    const isPlayerOneTurn = currentTurn === PLAYER_ONE;
    const isPlayerTwoTurn = currentTurn === PLAYER_TWO;
    const shouldMoveAI = (isPlayerOneTurn && GameState.getIsFirstPlayerAI()) ||
        (isPlayerTwoTurn && GameState.getIsSecondPlayerAI());
    const isGameOver = getWinner(GameState.getGameBoardState(), true, currentTurn);
...
export function checkGameStateAndStartNextTurn(shouldCheckWinner = false, maybeMoveAI) {
    let winner;
    ...
    if (winner) {
        ...
        console.log(message, `Number of turns taken: ${GameState.getNumberOfTurnsIntoGame()}`);
...
export function initGame(isHumanFirstPlayer) {
