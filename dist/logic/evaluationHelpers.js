...
export function getHasAllThreePieceTypes(gameState) {
...
export function isAnyPieceCapturable(gameState, player) {
    for (let i = 0; i < PLAYABLE_VERTICES.length; i++) {
...
export function canCaptureAnyPiece(gameState, player) {
    for (let i = 0; i < PLAYABLE_VERTICES.length; i++) {
        const coordinate = PLAYABLE_VERTICES[i];
        const piece = gameState[coordinate];
        if (piece &&
...
export function getWinner(gameState, beforeTurnStart, turn) {
