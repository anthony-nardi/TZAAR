...
import { getValidCaptures, getValidStacks, getInvertedValidCaptures, getValidStacksAndCaptures, } from "./gameBoardHelpers";
...
function getFirstTurnMoveSequence(gameState, turn) {
    const moves = [];
    const playerPiecesToCapture = turn === PLAYER_ONE ? PLAYER_TWO : PLAYER_ONE;
    const allOpponentPlayerPieces = getAllPlayerPieceCoordinatesByType(gameState, playerPiecesToCapture, TZAAR);
    ...
    for (let opponentCoordinateIndex = 0; opponentCoordinateIndex < allOpponentPlayerPieces.length; opponentCoordinateIndex++) {
        ...
        for (let captureCoordinateIndex = 0; captureCoordinateIndex < captureCoordinates.length; captureCoordinateIndex++) {
...
export function getEarlyGameMoveSequences(gameState, turn, type) {
    const moves = [];
    const playerPiecesToCapture = turn === PLAYER_ONE ? PLAYER_TWO : PLAYER_ONE;
    // Get all of the opponents most valuable pieces
    const allOpponentPlayerPieces = getAllPlayerPieceCoordinatesByType(gameState, playerPiecesToCapture, type);
    // If opponent has any stacks > 1 it is worth attempting to capture...
    // need to specifically look at the stacks to see if they can be captured.
    // If opponent has <= 2 pieces of a type we need to see if we can capture both.
    // Never pass
    ...
    for (let opponentCoordinateIndex = 0; opponentCoordinateIndex < allOpponentPlayerPieces.length; opponentCoordinateIndex++) {
        ...
        for (let captureCoordinateIndex = 0; captureCoordinateIndex < captureCoordinates.length; captureCoordinateIndex++) {
            ...
            const validMovesForPieceJustMoved = getValidStacksAndCaptures(toCoordinate, gameState);
            for (let validMoveIndex = 0; validMoveIndex < validMovesForPieceJustMoved.length; validMoveIndex++) {
                const moveString = sequenceKey +
                    "=>" +
                    toCoordinate +
                    "->" +
            ...
            const allOpponentPlayerPiecesWithHighStacks = getAllPlayerPieceCoordinatesByType(gameState, playerPiecesToCapture, type, 2);
            ...
            for (let j = 0; j < allPlayerPiecesOfType.length; j++) {
                ...
                for (let validMoveIndex = 0; validMoveIndex < validMoves.length; validMoveIndex++) {
                    const moveString = sequenceKey +
                        "=>" +
...
export function getAllPlayerPieceCoordinatesByType(gameState, player, type, stackSize = 1) {
    let coordinates = [];
    for (let i = 0; i < PLAYABLE_VERTICES.length; i++) {
        const coordinate = PLAYABLE_VERTICES[i];
        const piece = gameState[coordinate];
...
export function getPossibleMoveSequences(gameState, turn) {
    ...
    for (let playerCoordinateIndex = 0; playerCoordinateIndex < playerCoordinates.length; playerCoordinateIndex++) {
        ...
        for (let captureCoordinateIndex = 0; captureCoordinateIndex < captureCoordinates.length; captureCoordinateIndex++) {
            ...
            // for the piece we just moved... lets find ITS moves... (this is optimization so it looks dumb)
            // if its not obvious, only 2 coordinates on the board changed so we can reuse most of the previous results from getAllPlayerPieceCoordinates
            const validMovesForPieceJustMoved = getValidStacksAndCaptures(coordinateToCapture, gameState);
            const firstCaptureAndSecondStartingPosition = fromToKey + "=>" + coordinateToCapture + "->";
            for (let validMoveIndex = 0; validMoveIndex < validMovesForPieceJustMoved.length; validMoveIndex++) {
                const moveString = firstCaptureAndSecondStartingPosition +
                    validMovesForPieceJustMoved[validMoveIndex];
                moves.push(moveString);
            }
            for (let j = 0; j < playerCoordinates.length; j++) {
                ...
                const validMoves = getValidStacksAndCaptures(playerPieceCoordinateAfterCapture, gameState);
                const firstCaptureAndSecondStartingPosition2 = fromToKey + "=>" + playerPieceCoordinateAfterCapture + "->";
                for (let validMoveIndex = 0; validMoveIndex < validMoves.length; validMoveIndex++) {
                    const moveString = firstCaptureAndSecondStartingPosition2 + validMoves[validMoveIndex];
                    moves.push(moveString);
                }
...
//# sourceMappingURL=availableMovesGenerator.js.map