...
import { getPixelCoordinatesFromUserInteraction, getBoardCoordinatesFromUserInteraction, } from "../rendering/coordinateHelpers";
...
export function capturePiece(fromCoordinates, toCoordinates) {
    ...
    if (isDebugModeOn()) {
        if (GameState.getTurnPhase() === CAPTURE) {
            GameHistory.addFirstHumanMoveToCurrentGame(`${fromCoordinates}->${toCoordinates}`);
        }
        else {
            GameHistory.addSecondHumanMoveToCurrentGame(`${fromCoordinates}->${toCoordinates}`);
        }
    }
...
export function stackPiece(fromCoordinates, toCoordinates) {
    const gameBoardState = GameState.getGameBoardState();
    const fromPiece = gameBoardState[fromCoordinates];
...
export function handlePassTurn() {
    const canPass = GameState.getCurrentTurn() === PLAYER_ONE &&
