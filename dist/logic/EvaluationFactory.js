import { TZAAR, TOTT, TZARRA, PLAYER_ONE, PLAYER_TWO, EDGE_COORDINATES_AS_MAP, CORNER_COORDINATES_AS_MAP, PLAYABLE_VERTICES, } from "../constants";
import GameState from "./GameState";
export default class EvaluationFactory {
    constructor(props) {
        this.STACK_VALUE_BONUS = 15;
        this.EDGE_PENALTY = -1;
        this.CORNER_PENALTY = -2;
        this.LARGEST_STACK_BONUS = 1;
    ...
    getGameStateScore(node) {
        ...
        const minimizingPlayer = maximizingPlayer === PLAYER_ONE ? PLAYER_TWO : PLAYER_ONE;
        const gameMetadata = this.getGameStateMetadata(gameState);
        const maximizingPlayerScore = this.getTotalScore(gameMetadata, maximizingPlayer);
    ...
    getTotalScore(gameMetadata, player) {
    ...
    getScoreForStacks(numberOfPieces, stackSize) {
        return (this.STACK_VALUE_BONUS - numberOfPieces) * stackSize;
    }
    getGameStateMetadata(gameState) {
        ...
        const player1StacksOnEdge = [];
        const player1StacksOnCorner = [];
        const player2StacksOnEdge = [];
        const player2StacksOnCorner = [];
...
//# sourceMappingURL=EvaluationFactory.js.map