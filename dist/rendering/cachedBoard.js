...
function initCanvas() {
    ...
    cachedCanvasContext.setTransform(WindowHelper.devicePixelRatio, 0, 0, WindowHelper.devicePixelRatio, 0, 0);
    gameStateCanvasContext.setTransform(WindowHelper.devicePixelRatio, 0, 0, WindowHelper.devicePixelRatio, 0, 0);
}
function getContext() {
...
function getImageData() {
    ...
    cachedBoardImageData = context.getImageData(OFFSET_X * WindowHelper.devicePixelRatio, 0, Math.floor(WindowHelper.devicePixelRatio * WindowHelper.width), Math.floor(WindowHelper.devicePixelRatio * WindowHelper.height));
    return cachedBoardImageData;
}
export function drawCachedBoard() {
    const context = window.GAME_STATE_BOARD_CANVAS &&
    ...
    context.putImageData(imageData, Math.floor((WindowHelper.width / 2 - 4 * GamePieceRenderer.TRIANGLE_SIDE_LENGTH) *
        WindowHelper.devicePixelRatio), Math.floor((WindowHelper.height / 2 - 4 * GamePieceRenderer.TRIANGLE_HEIGHT) *
...
function renderTriangleFromVertex(coordinate) {
    ...
    const numX = Number(x);
    const numY = Number(y);
    const offsetX = (numY * GamePieceRenderer.TRIANGLE_SIDE_LENGTH) / 2 -
...
//# sourceMappingURL=cachedBoard.js.map