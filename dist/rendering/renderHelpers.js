...
import { PLAYABLE_VERTICES, TOTT, TZAAR, TZARRA, PLAYER_ONE, PLAYER_TWO, } from "../constants";
...
export function drawCoordinate(coordinate) {
    ...
    if (!GamePieceRenderer.TRIANGLE_HEIGHT ||
...
export function drawStaticGamePiece(gamePiece, coordinate) {
    ...
    const [xPos, yPos] = getPixelCoordinatesFromBoardCoordinates(coordinate).split(",");
...
export function drawGamePiece(gamePiece, xPos, yPos) {
    ...
    if (!GamePieceRenderer.GAME_PIECE_RADIUS ||
...
export function renderInitializingBoard(piecesToDraw, callback) {
    ...
    renderMovingPieces(piecesToRenderList, 500, Date.now(), () => {
        ...
        for (const coordinate in piecesToDraw) {
            const piece = piecesToDraw[coordinate];
            GameState.getGameBoardState()[coordinate] = piece;
            index = index + 1;
        }
        callback();
    });
...
function renderMovingPieces(piecesToRenderList, duration, startTime, callback) {
    ...
    piecesToRenderList.forEach(({ piece, from, to, delay, }) => {
        const timePassed = Math.min(Math.max((now - startTime - delay) / duration, 0), 1);
...
export function renderMovingPiece(piece, from, to, duration, startTime, callback) {
    const now = Date.now();
    const timePassedInMilliSec = now - startTime;
